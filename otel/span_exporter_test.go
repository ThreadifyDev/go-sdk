...
func TestExternalReferenceSelection(t *testing.T) {
	disabled := false
	for _, tc := range []struct {
		...
		{name: "explicit precedence", attrs: []attribute.KeyValue{attribute.String("workflow.run_id", "run"), attribute.String("threadify.external_ref", " explicit ")}, want: "explicit"},
		{name: "disabled", attrs: []attribute.KeyValue{attribute.String("workflow.run_id", "run")}, opt: &disabled},
		{name: "explicit with disabled", attrs: []attribute.KeyValue{attribute.String("threadify.external_ref", "explicit")}, opt: &disabled, want: "explicit"},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got, err := exporter.externalRef(span)
